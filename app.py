...
@limiter.limit('3 per minute')
def resend_code():
    print("Resend code called")  # Debug
    ...
    if 'password_reset_email' in session:
        ...
        print(f"Redirecting to change_password, session has email: {'password_reset_email' in session}")  # Debug
        return redirect(url_for('change_password'))

    elif 'pending_user' in session:
    ...
    flash('No pending verification found. Please try again.', 'warning')
    return redirect(url_for('login'))
